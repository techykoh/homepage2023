	...
	<ul>
		<c:choose>
			...
			<c:when test="${step eq '6'}">
				<li>
					<h3>c:forEach Tag : 기본 반복 태그, 고정된 획수 또는 초과 수집 동안 중첩된 본문 내용을 반복</h3>
					<h4>기본문법 : <br/>
			...
			<c:when test="${step eq '8'}">
				<li>
					...
					URL 2 : <c:url var="link2" value="/temp/jstlImport.do">
							...
							<br/>
					<!-- 단일 URL인 경우는 링크를 바로 삽입할 때가 많다.		
					<a href="https://www.naver.com" target="_blank">링크 1번</a></br/>		 -->
					<a href="${link1}" target="_blank">링크 1번</a></br/>
					<a href="${link2}" target="_black">링크 2번</a>
					<%-- 아래 코드는 비추입니다.
					<a href="/temp/jstlImport.do?test=${테스트 호출}" target="_black">링크 2번</a> --%>
				</li>
			</c:when>
			...
			<c:when test="${step eq '11'}">
				<li>
					<h3>fn:startsWith() : 주어진 문자열이 특정 문자열 값으로 시작하는지 확인하는데 사용</h3>

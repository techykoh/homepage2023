...
접 속 계 정 : <c:out value="${USER_INFO.id}"/>
		...
		<div id="bbs_search">
			<form name="frm" method="post" action="/board/selectList.do">
				<fieldset>
					...
					<input name="searchKeyword" value="<c:out value="${searchVO.searchKeyword}"/>" type="text" class="inp_s" id="inp_test" />
		...
		<div id="bbs_wrap">
			...
			<div class="bss_list">
				<table class="list_table">
					<thead>
						<tr>
							<td class="num" scope="col">번호</td>
							<td class="tit" scope="col">제목</td>
							<td class="writer" scope="col">작성자</td>
							<td class="date" scope="col">작성일</td>
							<td class="hits" scope="col">조회수</td>
						</tr>
					</thead>
					<tbody>
						<%-- 공지 글 --%>
							<c:forEach var="result" items="${noticeResultList}" varStatus="status">
								<tr class="notice">
									...
									<td class="tit">
										<c:url var="viewUrl" value="/board/select.do${_BASE_PRARM}">
											<c:param name="boardId" value="${result.boardId}"/>
											<c:param name="pagaIndex" value="${searchVO.pageIndex}" />
										</c:url>
